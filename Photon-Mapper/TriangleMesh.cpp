...
std::unique_ptr<TriangleMesh> TriangleMesh::fromOBJ(
    ...
    for (const auto& f : data.tris) {
        Point v0 = data.positions[f.pi[0]] * scaleFactor + (Coordinate)(offset);
        Point v1 = data.positions[f.pi[1]] * scaleFactor + (Coordinate)(offset);
        Point v2 = data.positions[f.pi[2]] * scaleFactor + (Coordinate)(offset);


        bool hasVN = !data.normals.empty() && f.ni[0] >= 0 && f.ni[1] >= 0 && f.ni[2] >= 0;

        if (smoothNormals && hasVN) {
